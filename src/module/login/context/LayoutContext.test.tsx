...
import {LayoutProvider, useLayout} from "@/module/login/context/LayoutContext";
...
jest.mock("@/common/provider/logging.provider", () => ({
...
describe("LayoutContext", () => {
    ...
    it("should set dashboardAccess to true if getUser is successful", async () => {
        ...
        await waitFor(() => {
            expect(queryByTestId("test-context-loading")).toHaveTextContent("true");
            expect(queryByTestId("test-context-access")).toHaveTextContent("true");
        });
    ...
    it("should call redeemAccountSession if getUser fails and query params are present", async () => {
        ...
        await waitFor(() => {
            expect(queryByTestId("test-context-loading")).toHaveTextContent("false");
            expect(queryByTestId("test-context-access")).toHaveTextContent("true");
        });
    ...
    it("should redirect to login if getUser fails and no query params are present", async () => {
        ...
        await waitFor(() => {
            expect(queryByTestId("test-context-loading")).toHaveTextContent("false");
            expect(queryByTestId("test-context-access")).toHaveTextContent("false");
        });
